...
Assert-Contains $codexSource '_r_obj_isstringempty \(ptr_log->path\) \|\| _r_obj_isstringempty \(ptr_log->user_sid\)' 'PID correlation must require WFP path and SID evidence.'
Assert-Contains $codexSource 'if \(_r_obj_isstringempty \(buffer\)\)\s+return;' 'Audit writes must ignore empty formatted records.'
